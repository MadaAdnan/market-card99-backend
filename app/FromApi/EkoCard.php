...
class EkoCard implements PayByApi
{
    private $setting;
    ...
    public function buyFromApiFree(Bill $bill,$id=null): Bill
    {
        ...
        $url = 'https://api.ekostore.co/client/api/newOrder/' . $bill->product->codes_api['eko'] . '/params?qty=' . $bill->amount . '&order_uuid=' . $code . '&playerID=' . $bill->customer_id;
    ...
    public function buyFromApiFixed(Bill $bill,$code=null): Bill
    {
        ...
        $url = 'https://api.ekostore.co/client/api/newOrder/' . $bill->product->codes_api['eko'] . '/params?qty=' . $bill->product->count . '&order_uuid=' .$code . '&playerID=' . $bill->customer_id;
        $token = $this->setting->apis['eko'];
        $bill->save();
        try {
            $response = \Http::withHeaders([
    ...
    {
        $url= 'https://api.ekostore.co/client/api/check?orders=[' . $bill->api_id . ']';
        $token=$this->setting->apis['eko'];
        $response = \Http::withHeaders([
            'api-token' => $token
        ])->get($url);
