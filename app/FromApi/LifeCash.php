...
class LifeCash implements PayByApi
{
    ...
    public function buyFromApiFree(Bill $bill, $id = null): Bill
    {
        ...
        $url = 'https://api.life-cash.com/client/api/newOrder/' . $bill->product->code . '/params?qty=' . $bill->amount . '&order_uuid=' . $code . '&playerID=' . $bill->customer_id;
    ...
    public function buyFromApiFixed(Bill $bill, $id = null): Bill
    {
        ...
        $url = 'https://api.life-cash.com/client/api/newOrder/' . $bill->product->code . '/params?qty=' . $bill->product->count . '&order_uuid=' . $code . '&playerID=' . $bill->customer_id;
    ...
    {
        $url = 'https://api.life-cash.com/client/api/check?orders=[' . $bill->api_id . ']';
        $token = $this->setting->apis['life'];
        $response = \Http::withHeaders([
            'api-token' => $token
        ])->get($url);
