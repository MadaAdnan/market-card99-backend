...
class SpeedCard implements PayByApi
{
    ...
    public function buyFromApiFree(Bill $bill, $code = null): Bill
    {
        ...
        $url = 'https://api.speedcard.vip/client/api/newOrder/' . $bill->product->codes_api['speed_card'] . '/params?qty=' . $bill->amount . '&order_uuid=' . $code . '&playerID=' . $bill->customer_id;
    ...
    public function buyFromApiFixed(Bill $bill, $code = null): Bill
    {
        ...
        $url = 'https://api.speedcard.vip/client/api/newOrder/' . $bill->product->codes_api['speed_card'] . '/params?qty=' . $bill->product->count . '&order_uuid=' . $code . '&playerID=' . $bill->customer_id;
        $token = $this->setting->apis['speed_card'];
        $bill->save();
        try {
            $response = \Http::withHeaders([
    ...
    {
        $url = 'https://api.speedcard.vip/client/api/check?orders=[' . $bill->api_id . ']';
        $token = $this->setting->apis['speed_card'];
