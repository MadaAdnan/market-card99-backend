...
class FastPva2 implements \App\InterFaces\ServerInterface
{
    ...
    public function getPhoneNumber(  Program $program): Order
    {
        ...
        $response = \Http::get('http://api.fastpva.com/pvapublic/sms/getNumber?myPid=' . $program->code . '&locale=' . trim($countryCode, 'any') . '&apikey=' . $server->api);
        ...
        if ($response->successful() && $response->json('code') == 1) {
            ...
            try {
                ...
                $order = Order::create([
                    ...
                    'server_id' => $server->id,
                    'api_id' => $response->json('data')['orderId'],
                    'status' => OrderStatusEnum::WAITE->value,
                    'phone' => $response->json('data')['number'],
                    'price' => $price,
                    'ratio' => $ratio,
                ]);
                \DB::commit();
    ...
    public function getPhoneCode(Order $order): string
    {
        ...
        $response = \Http::get('http://api.fastpva.com/pvapublic/sms/getCode?orderId=' . $order->api_id . '&apikey=' . $server->api);
        $time = now()->greaterThan($order->created_at->addMinutes(5));
        if ($response!=null && $response->successful() && isset($response->json()['code']) && $response->json('code') == 1) {
            $order->update([
        ...
        } elseif ($response->json('code') == -1 && !$time) {
