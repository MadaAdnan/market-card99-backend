...
class GrizzlySms implements \App\InterFaces\ServerInterface
{
    ...
    public function getPhoneNumber( Program $program): Order
    {
        ...
        $url = "https://api.grizzlysms.com/stubs/handler_api.php?api_key=" . $server->api . "&action=getNumber&service=" . $program->code . "&country=" . $countryCode;
        ...
        if ($response->successful() && \Str::contains($response->body(), 'ACCESS_NUMBER')) {
            ...
            try {
                ...
                Balance::create([
                    ...
                    'debit' => $price,
                    'credit' => 0,
                    'info' => 'طلب رقم ' . $app->name . ' من دولة ' . $cou->name,
    ...
    public function getPhoneCode(Order $order): string
    {
        ...
        $url = "https://api.grizzlysms.com/stubs/handler_api.php?api_key=" . $server->api . "&action=getStatus&id=" . $order->api_id;
