...
class Ndline implements \App\InterFaces\ServerInterface
{
    ...
    public function getPhoneNumber( Program $program): Order
    {
        ...
        $response = \Http::get('https://2ndline.io/apiv1/order?apikey=' . $server->api . '&countryId=' . $countryCode . '&serviceId=' . $program->code . '&operatorId=any');
        if ($response->successful() && $response->json('status') == 1) {
            sleep(5);
            $responsephone = \Http::get('https://2ndline.io/apiv1/ordercheck?apikey=' . $server->api . '&id=' . $response->json('id'));
            if ($responsephone->successful() && $responsephone->json('status') == 1) {
                \DB::beginTransaction();
                try {
                    $ratio=0;
                    ...
                    $order = Order::create([
                        ...
                        'phone' => $responsephone->json()['data']['phone'],
                        'price' => $price,
                        'ratio' => $ratio,
                    ]);
                    \DB::commit();
    ...
    public function getPhoneCode(Order $order): string
    {
        ...
        $response = \Http::get('https://2ndline.io/apiv1/ordercheck?apikey=' . $server->api . '&id=' . $order->api_id);
        $time = now()->greaterThan($order->created_at->addMinutes(5));
        if ($response->successful() && $response->json('status') == 1 && $response->json('data')['statusOrder'] == 1) {
            $order->update([
        ...
        } elseif (isset($response->json('data')['statusOrder']) && $response->json('data')['statusOrder'] == -0 && !$time) {
