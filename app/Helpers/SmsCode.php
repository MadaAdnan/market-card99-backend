...
class SmsCode implements \App\InterFaces\ServerInterface
{
    ...
    public function getPhoneNumber( Program $program): Order
    {
        ...
        $url ="https://sms-code.store/stubs/handler_api.php?api_key=" . $server->api . "&action=getNumber&service=" . $program->code . "&country=" . $countryCode ;
    ...
    public function getPhoneCode(Order $order): string
    {
        ...
        $url =  "https://sms-code.store/stubs/handler_api.php?api_key=" . $server->api . "&action=getStatus&id=" . $order->api_id;

        $response = \Http::get($url);
        $time = now()->greaterThan($order->created_at->addMinutes(10));
