...
class YmSms implements \App\InterFaces\ServerInterface
{
    ...
    public function getPhoneNumber(Program $program): Order
    {
        ...
        //$url="https://api.tqsms.xyz/api/getPhone?token={$server->api}&channelId=$countryCode";
        ...
        if ($response->successful() && isset($response->json('data')['smsTask'])) {
            ...
            try {
                ...
                $order = Order::create([
                    ...
                    'server_id' => $server->id,
                    'api_id' =>  /*$response->json('data')['smsTask']['id'],//,*/ $countryCode,
    ...
    public function getPhoneCode(Order $order): string
    {
        ...
        /*  $url="https://api.tqsms.xyz/api/getCode?token={$server->api}&channelId={$order->api_id}&phoneNum={$order->phone}";
          $response=\Http::get($url);*/
